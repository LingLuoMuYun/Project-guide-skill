...
- **深层分析 Phase 2** → Phase 1 完成后自动启动，输出 `project-guide/` 目录（交互式 HTML 指南，约 18 个文件）
...
深层分析 Phase 2 生成一个模块化的 **`project-guide/` 目录**（Phase 1 结束后自动启动，无需用户确认）。核心采用 **Section 注册表模式**：每个分析章节是独立文件，通过统一接口注册到核心框架。新增分析维度只需添加 section 文件 + 数据字段 + 一行 `<script>`。
