...
description: 分析、理解、记录或审计软件项目/仓库，并生成面向新成员的项目指南。浅层输出 Markdown 分析报告，深层输出交互式 HTML 指南（含关键业务代码展示与分析）。适用于快速接手陌生代码库、解释架构、梳理目录结构、生成项目结构树、分析代码文件内容、识别埋点与追踪、追踪核心流程、编写交接文档。
triggers:
  - 快速入手
  - 快速上手
  - 快速了解项目
