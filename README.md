...
| 🌐 **深层分析** ⭐ | `project-guide/index.html` | 交互式 HTML 指南 + 关键业务代码展示与逐行分析 |
| 🔍 **聚焦分析** | `project-guide/{模块名}-focus.md` 或 `.html` | 针对指定模块/功能深入分析，逐文件逐行剖析 |

激活技能后，AI 会先让你选择模式。用户可以直接说"聚焦分析 用户认证模块"一步到位。
...
**聚焦分析：** `聚焦分析` `针对xx模块` `聚焦xx功能` `深入分析xx` `只看xx模块` `局部深入` `模块分析` `focus on` `deep dive into`
...
分析下这个仓库的架构
聚焦分析 用户认证模块，输出 html
针对 src/services/ 做深入分析
只看支付流程
```

无需刻意记住 `/project-guide-skill` 命令，说出"分析项目""快速入手""聚焦分析xx模块"等关键词即可触发。
...
```text
# Claude Code — 一步到位
/project-guide-skill 聚焦分析 用户认证模块，输出 html
/project-guide-skill 针对 src/services/order/ 做深入分析
/project-guide-skill 只看支付流程，梳理清楚

# 或先触发 skill，再从菜单中选择"选项 3：聚焦分析"
帮我快速入手这个项目
→ AI 展示三个选项 → 选 3 → 告诉 AI 要聚焦的模块

# 通用方式（在任何 Agent 中）
[粘贴 PROMPT.md 内容后]
针对 src/modules/auth/ 进行聚焦分析，输出 html
```
